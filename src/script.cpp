...
static bool IsCanonicalPubKey(const valtype &vchPubKey) {
    if (vchPubKey.size() < 33U)
        return error("Non-canonical public key: too short");
    if (vchPubKey[0] == 0x04) {
        if (vchPubKey.size() != 65U)
...
bool EvalScript(vector<vector<unsigned char> >& stack, const CScript& script, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    try
    {
        while (pc < pend)
        {
            ...
            // Size limits
            if (stack.size() + altstack.size() > 1000U)
                return false;
        }
    }
    catch (...)
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    // Quickly identify that we have a poll and grab the bytes to validate through our poll system.
    if (scriptPubKey.IsVotePoll())
    {
        typeRet = TX_VOTEPOLL;
        vector<unsigned char> rawPoll(scriptPubKey.begin()+1, scriptPubKey.end());
        vSolutionsRet.push_back(rawPoll);
        return true;
    }

    // Quickly identify that we have ballots and grab the bytes to validate through our poll system.
    if (scriptPubKey.IsVoteBallots())
    {
        typeRet = TX_VOTEBALLOTS;
        vector<unsigned char> rawBallots(scriptPubKey.begin()+1, scriptPubKey.end());
        vSolutionsRet.push_back(rawBallots);
        return true;
    }
    ...
    BOOST_FOREACH(const PAIRTYPE(txnouttype, CScript)& tplate, mTemplates)
    {
        ...
        while (true)
        {
            ...
            {
                while (vch1.size() >= 33U && vch1.size() <= 120U)
                {
            ...
            {
                if (vch1.size() < 33U || vch1.size() > 120U)
                    break;
                vSolutionsRet.push_back(vch1);
            }
...
int ScriptSigArgsExpected(txnouttype t, const std::vector<std::vector<unsigned char> >& vSolutions)
{
    ...
    case TX_PAY2POLL:
        return 3;
    case TX_MULTISIG:
        if (vSolutions.size() < 1U || vSolutions[0].size() < 1U)
            return -1;
...
bool IsMine(const CKeyStore &keystore, const CScript& scriptPubKey)
{
    ...
    case TX_VOTEPOLL:
    {
        const unsigned char *tempScript = scriptPubKey.data();
        vector<unsigned char> vchPollID(&tempScript[1U+POLL_INFO_SIZE], &tempScript[1U+POLL_INFO_SIZE + POLL_ID_SIZE]);
        // memcpy(&vchPollID, &tempScript[1U+POLL_INFO_SIZE], POLL_ID_SIZE);
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CTransaction& txTo, unsigned int nIn,
                  int nHashType)
{
    // Lets get this one over quick.
    if (scriptSig.IsVotePoll())
        return false;

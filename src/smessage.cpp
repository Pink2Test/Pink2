...
void SecMsgBucket::hashBucket()
{
    ...
    if (fDebugSmsg)
        printf("Hashed %zu messages, hash %u\n", setTokens.size(), hash);
};
                ...
                } else
                {
                    ...
                    if (it->second.nLockCount > 0)
                    {
                        ...
                        if (it->second.nLockCount == 0)     // lock timed out
                        {
                            ...
                            if (fDebugSmsg)
                                printf("Lock on bucket %ld for peer %u timed out.\n", it->first, nPeerId);
                            ...
                            BOOST_FOREACH(CNode* pnode, vNodes)
                            {
                                ...
                                if (fDebugSmsg)
                                    printf("This node will ignore peer %u until %ld.\n", nPeerId, ignoreUntil);
                                break;
                            };
...
int SecureMsgBuildBucketSet()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        if (fDebugSmsg)
            printf("Bucket %ld contains %zu messages.\n", fileTime, tokenSet.size());
    };
    
    printf("Processed %u files, loaded %zu buckets containing %u messages.\n", nFiles, smsgBuckets.size(), nMessages);
    ...
    */
    ...
    if (strCommand == "smsgInv")
    {
        ...
        for (uint32_t i = 0; i < nInvBuckets; ++i)
        {
            ...
            {
                if (fDebugSmsg)
                    printf("Peer sent empty bucket, ignore %ld %u %u.\n", time, ncontent, hash);
                continue;
            };
    ...
    if (strCommand == "smsgShow")
    {
        ...
        for (uint32_t i = 0; i < nBuckets; ++i, pIn += 8)
        {
            ...
            try {
                ...
                printf("vchDataOut.resize %zu threw: %s.\n", 8 + 16 * tokenSet.size(), e.what());
                continue;
            };
    ...
    if (strCommand == "smsgHave")
    {
        ...
        {
            if (fDebugSmsg)
                printf("Bucket %ld lock count %u, waiting for message data from peer %u.\n", time, smsgBuckets[time].nLockCount, smsgBuckets[time].nLockPeerId);
            return false;
        }; 
        
        if (fDebugSmsg)
            printf("Sifting through bucket %ld.\n", time);
        ...
        if (vchDataOut.size() > 8)
        {
            if (fDebugSmsg)
            {
                printf("Asking peer for  %zu messages.\n", (vchDataOut.size() - 8) / 16);
    ...
    if (strCommand == "smsgMsg")
    {
        ...
        if (fDebugSmsg)
            printf("smsgMsg vchData.size() %zu.\n", vchData.size());
    ...
    if (strCommand == "smsgMatch")
    {
        ...
        if (vchData.size() < 8)
        {
            printf("smsgMatch, not enough data %zu.\n", vchData.size());
            pfrom->Misbehaving(1);
            return false;
        };
    ...
    if (strCommand == "smsgIgnore")
    {
        ...
        if (vchData.size() < 8)
        {
            printf("smsgIgnore, not enough data %zu.\n", vchData.size());
            pfrom->Misbehaving(1);
            return false;
        };
        ...
        if (fDebugSmsg)
            printf("Peer %u is ignoring this node until %ld, ignore peer too.\n", pfrom->smsgData.nPeerId, time);
    } else
    {
        // Unknown message
    };
        ...
        if (nBuckets > 0) // no need to send keep alive pkts, coin messages already do that
        {
            ...
            for (it = smsgBuckets.begin(); it != smsgBuckets.end(); ++it)
            {
                ...
                try {
                    ...
                    printf("vchData.resize %zu threw: %s.\n", vchData.size() + 16, e.what());
                    continue;
                };
                memcpy(p, &it->first, 8);
...
int SecureMsgReceive(CNode* pfrom, std::vector<unsigned char>& vchData)
{
    ...
    {
        printf("Error: Invalid no. messages received in bunch %u, for bucket %ld.\n", nBunch, bktTime);
        pfrom->Misbehaving(1);
...
int SecureMsgStore(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload, bool fUpdateBucket)
{
    ...
    
    {
        ...
        if (it != tokenSet.end())
        {
            ...
            if (fDebugSmsg)
            {
                ...
                printf("message ts: %ld", token.timestamp);
    ...
    //if (fDebugSmsg)
    printf("SecureMsg added to bucket %ld.\n", bucket);
    return 0;
};
...
int SecureMsgSetHash(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload)
{
    ...
    */
    ...
    {
        if (fDebugSmsg)
            printf("SecureMsgSetHash() failed, took %ld ms, nonse %u\n", GetTimeMillis() - nStart, nonse);
        return 1;
    };
    ...
    if (fDebugSmsg)
        printf("SecureMsgSetHash() took %ld ms, nonse %u\n", GetTimeMillis() - nStart, nonse);
